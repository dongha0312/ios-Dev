...
struct ExchangeRequestErrorDetail{
    let error: Error
...
class ExchangeRateDelegate: ExchangeRateResponseDelegate, ObservableObject{
    ...
    @Published var errorDetail: ExchangeRequestErrorDetail? = nil
    ...
    func requestFailedWith(error: (any Error)?, type: ExchangeRateRequestErrorType) {
        DispatchQueue.main.async{
            ...
            if let err = error{
                self.errorDetail = ExchangeRequestErrorDetail(error: err, type: type)
            }
